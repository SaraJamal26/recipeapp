...
import {
  ...
  TextInput, 
  StyleSheet, 
  Alert, Animated, DeviceEventEmitter
} from 'react-native';
...
const EditProfileScreen = () => {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem('profilepicture').then(pic => {if (pic!=null) setProfilePicture(pic)})
  ...
  const updatePicture = (newPicture: string) => {
    ...
    if (newPicture) {
      ...
      setImage('data:image/png;base64,' + newPicture)
  ...
  const uploadImage = async () => {
    ...
    const task = storageRef.putString(profilePicture, 'base64')
...
//       const imageUri = Platform.OS === 'ios' ? image.sourceURL : image.path;
//       setImage(imageUri);
//       const thi = this.bs.current.snapTo(1)
//       if (this.bs.current!=undefined)
//       this.bs.current.snapTo(1);
//     });
//   };
